...
export async function listProjectSectorReports({ projectId, sector }) {
  ...
  for (const object of objects) {
    ...
    if (key.endsWith("/library-state.json")) continue;

    const meta = await getJson(key).catch(() => null);
    if (!meta || typeof meta !== "object") continue;

    items.push({
      ...meta,

      // For migrated legacy metadata, trust the project-specific folder path.
      projectId: normalizeProjectId(meta.projectId) || scope.projectId,
      sector: normalizeSector(meta.sector) || scope.sector,

      // Always expose the actual project-scoped metadata key.
      metaKey: key
    });
  }

  items.sort((a, b) =>
    String(b.addedAt || b.createdAt || b.updatedAt || "").localeCompare(
      String(a.addedAt || a.createdAt || a.updatedAt || "")
    )
  );
